...
func get_complementary_version_data(downloader: Requests, version_id: StringName):
	...
	var path = get_forge_version_file()
...
func install_forge(downloader: Requests, minecraft_folder: String, java_path: String):
	...
	if FileAccess.file_exists(get_forge_version_file()) and FileAccess.file_exists(installer_folder.path_join(install_check_file)):
		print_debug("Forge is already installed")
		return #Forge already installed
	
	#install_forge_thread = Thread.new()
	#install_forge_thread.start(_install_forge.bind(i_p, java_path, minecraft_folder, install_check_file))
	_install_forge(i_p, java_path, minecraft_folder, install_check_file)

# in threads
func _install_forge(i_p, java_path, minecraft_folder, install_check_file):
	printt("threads", i_p, java_path, minecraft_folder, install_check_file)
	var installer_folder = i_p.get_base_dir()
